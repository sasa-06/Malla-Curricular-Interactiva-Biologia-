    ...
    const renderMalla = () => {
        ...
        for (const semesterNum in semesters) {
            ...
            semesters[semesterNum].forEach(course => {
                ...
                const coreqMet = course.coreq.every(coreq => courses.find(c => c.name === coreq && (approvedCourses.includes(c.name) || (c.semester === course.semester && course.prereq.every(p => approvedCourses.includes(p))))));
                
                const isAvailable = prereqMet && coreqMet;
